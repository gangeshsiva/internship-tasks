...
def vertexAI_endpoint(context, context_length, SYSTEM_PROMPT=Date_extraction_instructions,logfile=config_data["logfile"]):
    try:
        url = config_data["vertex_url"]  # GET FROM CONFIG
...
def ICD_ICP_code(all_text, words_coordinates,documentDetailId,environment, logfile):
    try:
                ...
                for icd in corrected_icd:
                    if icd in ICDfilteredjson:
                        ...
                        if ICDPagenum not in codePages:
                            ...
                            context= text+all_text[ICDPagenum - 1]
                            try:
                                ICD_data['date'] = vertexAI_endpoint(context , context_length = context_length)
                            except Exception as e:
                                print(f"Error occurred while fetching date for ICD {icd}: {e}")
                                ICD_data['date'] = "1900-01-01"
                ...
                for cpt in cptmatch:
                    if cpt.isdigit() and len(cpt) == 5 and cpt in CPTfilteredjson:
                        ...
                        if CPTPagenum not in codePages:
                            ...
                            context= text+all_text[CPTPagenum - 1]
                            try:
                                CPT_data['date'] = vertexAI_endpoint(context , context_length = context_length)
                            except Exception as e:
                                print(f"Error occurred while fetching date for CPT {cpt}: {e}")
                                CPT_data['date'] = "1900-01-01"
                    ...
                    for c,v in Snomedfilteredjson.items():
                        if sct.isdigit() and 6 <= len(sct) <= 18 and sct==c:
                            ...
                            if SCTPagenum not in codePages:
                                ...
                                context= text+all_text[SCTPagenum - 1]
                                try:
                                    SCT_data['date'] = vertexAI_endpoint(context , context_length = context_length)
                                except Exception as e:
                                    print(f"Error occurred while fetching date for SCT {sct}: {e}")
                                    SCT_data['date'] = "1900-01-01"
