...
logfile='C:\\Users\\gangeshvar.s\\Desktop\\langchain\\logfile.txt'
...
# Usage
file_path = "C:\\Users\\gangeshvar.s\\Desktop\\langchain\\retrievedoutput.json"
...
def langchainrag(retrieved_data,logfile):
    try:
        ...
        for t in texts:
            split_parts = splitter.split_text(t)
            all_chunks.extend(split_parts)

        # 3) Use Ollama embeddings (default: "nomic-embed-text")
        emb = OllamaEmbeddings(model="nomic-embed-text")
        print(all_chunks)
        # 4) Build FAISS vectorstore
        vectorstore = FAISS.from_texts(all_chunks, emb)
        ...
        pagecheck=answer.lower()
        page_numbers = []

        for chunk in docs:
            text = chunk["text"].lower()
            if pagecheck in text:     # simple keyword matching
