...
def match_coordinates(output, word_coordinates, logfile = logfile):
    try:
        new_data = []
        reg = r'[^A-Za-z0-9]'
        for data in output:
...
def ICD_CPT_SCT_code(all_text, words_coordinates, logfile):
    try:
    ...
    ICDpattern=r"\b(?<!\d{2})(?<!\d{2}[ ])(?<!mo\s)(?<!A:\s)(?<!-)(?<!BMI:\s)[A-Z0-9]{1,4}[.,][A-Z0-9]{1,4}\b(?!\s*(?:AM|PM|am|pm|m2|kg|F|/h|years|PST|PDT|\.|Ibs|cm|T|Fo|C|bpm|mL|mg|-|Wt|in|/|%))"   #(?:ICD[-\s]*10\s*)?
    ICDpattern2=r"\b(?<!\d{2})(?<!\d{2}[ ])(?<!mo\s)(?<!A:\s)(?<!-)(?<!BMI:\s)[A-Z][0-9]{2}\b(?!\s*(?:AM|PM|am|pm|m2|kg|F|/h|years|PST|PDT|\.|Ibs|cm|T|Fo|C|bpm|mL|mg|-|Wt|in|/|%))"
    ICDpattern3=r"\b(?<!\d{2})(?<!\d{2}[ ])(?<!mo\s)(?<!A:\s)(?<![-#])(?<![A-Z][ ])(?<!BMI:\s)[A-Z0-9]{4,7}\b(?!\s*(?:AM|PM|am|pm|m2|kg|F|/h|years|PST|PDT|\.|Ibs|cm|T|Fo|C|bpm|mL|mg|-|Wt|in|/|%|[A-Z]{2}|[A-Z][ ]))"
                ...
                for sct in sctmatch:
                    for c,v in Snomedfilteredjson.items():
                        if sct.isdigit() and 6 <= len(sct) <= 18 and sct==c:
                            ...
                            SCT_data['date'] = formatted_date
                            pair = (SCT_data["code"], SCT_data["date"])
